function meanField=spatialMeanAveraged_tt(S,R,xy,tt1,Lo,Hi,recTypeFlg,showFig)
...
[~, Ntime]=size(tt1);
tt=mean(tt1,2); % take average over time and use for the mean field estimates
...
for n=1:Ntime
    if recTypeFlg==1
     dtt=tt1(:,n)-tt; % travel time due to fluct relative to time-averaged fields: t-<t> 
    else
     dtt=tt1(:,n)-tt0; % travel time due to fluctuations at each time
    end
    meanField(n).c=c0est;
    %meanField.dc=dc;
    meanField(n).T=T0est;
    %meanField.dT=dT;
    meanField(n).vx=vx0est;
    %meanField.dvx=dvx;
    meanField(n).vy=vy0est;
    %meanField.dvy=dvy;
    
    meanField(n).std_dc=dc;
    meanField(n).std_dT=dT;
    meanField(n).std_dvx=dvx;
    meanField(n).std_dvy=dvy;
    %%%%%%%%%%%%%%%%%%%%%%%%% misc
    % meanField.std_tt=sig_tt;
    % meanField.std_x=sig_x;
    meanField(n).dtt=dtt;
    meanField(n).tt=tt1(:,n);
    meanField(n).index=in;
    meanField(n).data=[-c0est*c0est*dtt in];
    meanField(n).xy=xy;
    meanField(n).Li=Li;
    meanField(n).tt0=tt0;
end