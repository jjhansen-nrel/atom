function meanField=spatialMeanDelays(S,R,xy,tt,Lo,Hi,showFig)
...
lLi=length(Li);
G=zeros(lLi,3+S);
G(:,1:3)=[ones(lLi,1) -theta];
%% extra terms corresponding to delays fixed for each source and independent of ray's length
I=in<=R;
% G(I,4)=1./Li(I);
G(I,4)=1;
I=(in>R & in<=2*R);
