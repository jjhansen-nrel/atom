function [meanField,delays]=spatialMean_serial_maxD(S,R,xy,tt,Lo,Hi,delaysFlg,Nmax,showFig)
...
if delaysFlg
    tmp=~badInd;
    goodInd=find(tmp(4:end));
    delays(goodInd)=aver(4:end);
    ldel=length(goodInd);
    [~,i]=sort(abs(delays(goodInd)),1,'descend');
    ind=setdiff(1:ldel,i(1:Nmax));
% else
%     delays=zeros(S*R,1);
end
if ~isempty(ind)
...
%     ind=[1 10 12];
    ...
    G_1=pinv(G);
    aver=G_1*d;
   % delays(setdiff(1:SR,ind))=aver(4:end);
%     delays=zeros(ldel,1);
    delays(goodInd(i(1:Nmax)))=aver(4:end);
end
er=d-G*aver;
